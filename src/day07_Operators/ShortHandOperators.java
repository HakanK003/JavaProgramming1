package day07_Operators;

import java.sql.SQLOutput;

public class ShortHandOperators {

    public static void main(String[] args) {

        //assignment: =                           //We are using same variable but update the data

        int number = 100;                         //Updated version is at the bottom (Java reads codes up to bottom)

        System.out.println("number = " + number); // 100

        number = 200;

        System.out.println("number = " + number); //200

        String word = "Java Programing";

        System.out.println("word = " + word); //Java Programming

        word = "Wooden Spoon";                //We are using same variable but update the data

        System.out.println("word = " + word); //Wooden Spoon

        word = "Cydeo";

        System.out.println("word = " + word); //Cydeo

                                              //Code brings last version of variable (new versions comes after changes)
        // word = "123" + 1 ;   -1231
        // word = 123;          -will not work

        word = "Java Programming";

        System.out.println("word = " + word); //These variables can only have one data at same time

        System.out.println("-----------------------------------------------------------------------------------------");

        //Addition Assignment: +

        int x = 100;

        System.out.println("x = " + x);

        System.out.println(x + 200);         //300

        // x = x + 200;
        x += 200;                           //Shortcut for additions
        System.out.println("x = " + x);     //300

        String str ="Wooden";

        str +=" Spoon";

        System.out.println("str = " + str);

        double num1 = 2.5;

        System.out.println("num1 = " + num1);//2.5

        num1 +=5.5;

        System.out.println("num1 = " + num1);//8.0

        double availableBalance= 1000.50;

        //deposit 300$
        //availableBalance = 300;           //300
        availableBalance += 300;            // availableBalance = 1000.50 + 300

        System.out.println("availableBalance = " + availableBalance);//1300.50

        System.out.println("----------------------------------------------------------------------------------------");

        //withdrawing 500$

        //availableBalance += 500;          //1800.50
        availableBalance -= 500;            //800.50

        System.out.println("availableBalance = " + availableBalance);

        // *** we can't apply any shorthand operators to String (ony + we can)

        // withdrawing 200$, then depositing 400$

        availableBalance -= 200;            //600.50
        availableBalance += 400;            //1000.50

        System.out.println("availableBalance = " + availableBalance);//1000.50

        System.out.println("----------------------------------------------------------------------------------------");

        double salary = 50000.50;

        System.out.println("salary = " + salary);//50000.50

        salary *= 2;                        //100001

        System.out.println("salary = " + salary);//100001

        double dodge = 0.000000001;

        dodge *= 1000000;

        System.out.println("dodge = " + dodge);

        System.out.println("-----------------------------------------------------------------------------------------");

        double num2 = 25000;

        // num2 = num2 / 2;
        num2 /= 2;

        System.out.println("num2 = " + num2);

        System.out.println("-----------------------------------------------------------------------------------------");

        double num3 = 100;

        // %=

        num3 %= 3;              //1

        System.out.println("num3 = " + num3);//1

        System.out.println("-----------------------------------------------------------------------------------------");

        int amount = 127;       // cents

        int quarters = amount / 25 ;
        int cents = amount % 25 ;

        System.out.println("quarters = " + quarters);
        System.out.println("cents = " + cents);

        System.out.println("-----------------------------------------------------------------------------------------");

        int cents2 = 127;

        cents2 %= 25;

        System.out.println("cents2 = " + cents2);

        System.out.println("-----------------------------------------------------------------------------------------");

        int y = 300;

        y %= 16;

        System.out.println("y = " + y);

        System.out.println("-----------------------------------------------------------------------------------------");

        int balance = 3500;

        //insurance fee: $153

        balance %=153;

        System.out.println("balance = " + balance);




    }
}
