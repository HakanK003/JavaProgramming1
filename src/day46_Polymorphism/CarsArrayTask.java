...
import day43_Abstraction.carTask.*;

public class CarsArrayTask {
    ...
    public static void main(String[] args) {
        ...
        for (Car each : carsArray) {
            ...
            if(each instanceof BMW && (each.getYear()>=1929 && each.getYear()<=2022) ){
                System.out.println("This car can be recalled "+each);
            }

            if(each instanceof Toyota && (each.getYear()==2010 ||each.getYear()==2011) ){
                System.out.println("This car can be recalled "+each);
            }

            if (each.getMiles() > CarHasHighestMiles.getMiles()) {CarHasHighestMiles=each;}

            if (each.getMiles() < CarHasLowestMiles.getMiles() ) {CarHasLowestMiles=each; }

        }


        System.out.println("This car has highest miles " + CarHasHighestMiles);
        System.out.println("This car has lowest miles " + CarHasLowestMiles);
